...
var treload = $('#tabla-usuarios-manager').DataTable({
	dom: 'T<"clear">lfrtip',
        tableTools: {
	...
	ajax: { // configuración ajax
            "url": "settings/addusersmanager/listar.php",
            "type": "GET",
			"dataSrc": ""
    },
...
$('#nuevo-manager').on('shown.bs.modal', function (event) {
  $('#formularioNuevo-manager')[0].reset();	// reseamos el formulario para añadir un nuevo registro
  $('#formularioNuevo-manager [name=nombre]').focus();
});

// agregamos foco al compo de texto nombre en modal editar usuario , cuando la ventana modal este visible
$('#editar-manager').on('shown.bs.modal', function (event) {
  $('#edit_nombre').focus();
});

// funcion para recargar o refrescar la table
$(document).on("click", '#recarga-manager', function (event) {
 treload.ajax.reload();
});

//function para guardar usuario en la BD
$('#guardarNuevo-manager').click(function (event) {
var datos = $('#formularioNuevo').serialize();	// recuperamos todos los campos del formulario

			$.ajax ({ // configuraci{on de ajax
            type: "POST",
            url: "settings/addusersmanager/agregar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
...
$(document).on("click", '#actualizar-manager', function (event) {
var datos = $('#formularioEditar-manager').serialize(); // recuperamos todos los campos del formulario editar usuario
			$.ajax ({ // configuraci{on de ajax
            type: "POST",
            url: "settings/addusersmanager/actualizar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
				if(data.msg=='ok'){
					//cerramos la ventana modal editar usuario
					$('#editar-manager').modal('toggle');
...
function(){ // esta función se ejecuta cuando confirmamos  por si
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/addusersmanager/eliminar.php",
				data: { "cid" : cid },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
function(){
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/addusersmanager/estado.php",
				data: { "cid" : cid },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
$(document).on("click", '.editar', function (event) {
			var cid = $(this).attr ("cid"); // recuperamos la id del usuario	
			$('#id_user_manager').val(cid); // guardamos la id del usuario temporalmente en el campo oculto
			$.ajax ({ // configuración de ajax
            type: "POST",
            url: "settings/addusersmanager/editar.php",
            data: { "cid":cid },
			dataType: 'json'
        	}).done(function(data){
					// recuperamos y asignamos a los campos del formulario editar usuario
        			$('#formularioEditar-manager input[name="cid"]').val(data[0]['cid']); // selección por id y atributo
					$('#formularioEditar-manager input[name="name"]').val(data[0]['name']);
					$('#formularioEditar-manager input[name="email"]').val(data[0]['email']);
					$('#formularioEditar-manager input[name="phone"]').val(data[0]['phone']);
					$('#formularioEditar-manager input[name="office"]').val(data[0]['office']);
					$('#formularioEditar-manager input[name="role"]').val(data[0]['role']);
					$('#formularioEditar-manager input[name="pwd"]').val(data[0]['pwd']);
					if(data[0].estado=='1'){ // marcamos o desmarcamos el check box activo según estado del usuario
					$('#estado-manager').prop('checked', 'true');	
					
					}
					else{
					$('#estado-manager').removeAttr('checked');		
					}		
        	}); // fin de ajax
}); // fin del evento click
