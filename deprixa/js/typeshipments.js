...
var treload = $('#tabla-usuarios').DataTable({
	...
	ajax: { // configuración ajax
            "url": "settings/typeshipments/listar.php",
            "type": "GET",
			"dataSrc": ""
    },
...
var datos = $('#formularioNuevo').serialize();	// recuperamos todos los campos del formulario
            ...
            type: "POST",
            url: "settings/typeshipments/agregar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
...
var datos = $('#formularioEditar').serialize(); // recuperamos todos los campos del formulario editar usuario
			$.ajax ({ // configuraci{on de ajax
            type: "POST",
            url: "settings/typeshipments/actualizar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
				if(data.msg=='ok'){
...
$('.packaging').keypress(function(event){
	if($('.packaging').val().length>1){
	//quitamos la clase error
...
function(){ // esta función se ejecuta cuando confirmamos  por si
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/typeshipments/eliminar.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
function(){
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/typeshipments/estado.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
$(document).on("click", '.editar', function (event) {
			...
			$.ajax ({ // configuración de ajax
            type: "POST",
            url: "settings/typeshipments/editar.php",
            data: { "id":id },
			dataType: 'json'
        	}).done(function(data){
					...
					$('#formularioEditar input[name="packaging"]').val(data[0]['packaging']);
					$('#formularioEditar input[name="dimensions"]').val(data[0]['dimensions']);
