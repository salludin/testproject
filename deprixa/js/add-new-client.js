...
var treload = $('#tabla-usuarios').DataTable({
	...
	ajax: { // configuración ajax
            "url": "settings/add-new-clients/listar.php",
            "type": "GET",
			"dataSrc": ""
    },
...
var datos = $('#formularioNuevo').serialize();	// recuperamos todos los campos del formulario
            ...
            type: "POST",
            url: "settings/add-new-clients/agregar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
					...
					switch(data.msg){
						...
						case 'emavaci':
						notificar("Error the Email field is required.","error");
						//añadimos una clase error al campo nombre
						...
						case 'usuvacio':
						notificar("Error the User Timing field is required","error");
						//añadimos una clase error al campo nombre
						...
						case 'regvacio':
						notificar("Error the register field is required.","error");
						//añadimos una clase error al campo nombre
						$('#gregister').addClass('has-error');
...
var datos = $('#formularioEditar').serialize(); // recuperamos todos los campos del formulario editar usuario
			$.ajax ({ // configuraci{on de ajax
            type: "POST",
            url: "settings/add-new-clients/actualizar.php",
            data: datos,
			dataType:"json"
        	}).done(function(data){
				if(data.msg=='ok'){
...
swal({ // plugin sweet alert más información en: http://t4t5.github.io/sweetalert/
  ...
  text: "This User will be completely removed from the database", // texto del alert
...
function(){ // esta función se ejecuta cuando confirmamos  por si
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/add-new-clients/eliminar.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
function(){
	...
	$.ajax ({ // configuración de ajax
				type: "POST",
				url: "settings/add-new-clients/estado.php",
				data: { "id" : id },
				dataType:"json"
				}).done(function(data){
					if(data.msg=='ok'){
...
$(document).on("click", '.editar', function (event) {
			...
			$.ajax ({ // configuración de ajax
            type: "POST",
            url: "settings/add-new-clients/editar.php",
            data: { "id":id },
			dataType: 'json'
        	}).done(function(data){
					...
					$('#formularioEditar input[name="email"]').val(data[0]['email']);
					$('#formularioEditar input[name="password"]').val(data[0]['password']);
